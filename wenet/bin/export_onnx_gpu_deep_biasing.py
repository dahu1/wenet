...
class Encoder(torch.nn.Module):
    ...
    def forward(
        ...
        # logger.info("beam_log_probs: {}, beam_log_probs_idx: {}".format(beam_log_probs, beam_log_probs_idx))
        # logger.info("beam_log_probs.shape: {}, beam_log_probs_idx.shape: {}".format(beam_log_probs.shape, beam_log_probs_idx.shape))
        return (
            encoder_out,
            encoder_out_lens,
            ctc_log_probs,
...
class Context(torch.nn.Module):
    def __init__(self, context_module: ContextModule, ctc: CTC,
        biasing_score: float = 1.0  
    ):
        super().__init__()
        self.context_module = context_module
        self.ctc = ctc
        self.deep_biasing_score = biasing_score

    def forward(
        self, context_list: torch.Tensor, context_list_lengths: torch.Tensor,
        ...
        encoder_bias_out, _ = self.context_module(context_emb, encoder_out,
...
class Decoder(torch.nn.Module):
    ...
    def forward(
        ...
        if self.decoder_fastertransformer:
            return decoder_out, score, best_index
        else:
...
def get_context_list_tensor(context_list):
...
def export_context(model, logger, context_onnx_path, context_pt_path):
    biasing_score = 1.0
    context_module = Context(model.context_module, model.ctc, biasing_score)
    context_module.eval()

    # save context pt model
    if isinstance(context_module, torch.nn.DataParallel):
        state_dict = context_module.module.state_dict()
    elif isinstance(context_module, torch.nn.parallel.DistributedDataParallel):
        state_dict = context_module.module.state_dict()
    else:
        state_dict = context_module.state_dict()
    torch.save(state_dict, context_pt_path)
    # exit(0)

    # # context shape construct
    # # wb = 32
    # wb = 2
    # wlen = 8
    # context_list = np.random.randint(1, 5000, size = (wb, wlen))
    # # context_list = []
    ...
    # # t = 63
    # d = 512
    # encoder_out = torch.randn(b , t, d, dtype=torch.float32)
    # # xs: padded output tensor (B, T' ~= T/subsample_rate, D)


    # # torch.onnx.export(
    # #     context_module,
    # #     (context_list, context_list_lengths, encoder_out),
    # #     context_onnx_path,
    # #     export_params=True,
    # #     opset_version=13,
    # #     do_constant_folding=True,
    # #     input_names = ["context_list", "context_list_lengths", "encoder_out"],
    # #     output_names=[
    # #         "encoder_bias_out"
    # #     ],
    # #     dynamic_axes={
    # #         # "context_list": {0: "WB", 1: "WLEN"},
    # #         # "context_list_lengths": {0: "WB"},
    # #         "encoder_out": {0: "B", 1: "T"},
    # #         "encoder_bias_out": {0: "B", 1: "T"},
    # #     },
    # #     verbose=False,
    # # )

    # with torch.no_grad():
    #     o0 = context_module(context_list, context_list_lengths, encoder_out)
    # logger.info("o0.shape: {}".format(o0.shape))
    # # o0.shape: torch.Size([32, 300, 512])
    # assert o0.shape == encoder_out.shape

    # providers = ["CUDAExecutionProvider"]
    # ort_session = onnxruntime.InferenceSession( context_onnx_path, providers=providers)
    # # context_onnx = "/ssdata5/xuecheng/asr_tools/wenet_update/wenet_nn_bias/wenet/runtime/gpu/model_repo/context/1/context.onnx"
    # # context_onnx = "/ssdata5/xuecheng/asr_tools/wenet_update/wenet_nn_bias/wenet/examples/bias/s0/exp/nn_bias/onnx_dir_bias_tmp/context.onnx"
    # # context_session = onnxruntime.InferenceSession(context_onnx, providers=['CUDAExecutionProvider'])
    # context_list = context_list.numpy()
    # context_list_lengths = context_list_lengths.numpy()
    ...
    # # ort_outs = context_session.run(None, ort_inputs)
    # logger.info("see onnx inference shape")
    ...
    # # logger.info("o0: {}, shape: {}".format(o0, o0.shape))
    # # logger.info("o0 shape: {}".format(o0.shape))
    # # logger.info("o0[0] : {}".format(o0[0]))
    # logger.info("ort_outs: {} shape: {}".format(ort_outs, ort_outs[0].shape))
    # logger.info("encoder_out shape: {}".format(encoder_out.shape))

    # # check encoder output
    # test(to_numpy([o0]), ort_outs)
    # logger.info("export context onnx encoder succeed!")
...
def export_rescoring_decoder(
    ...
    # logger.info("score: {}, shape: {}".format(o0, o0.shape))
    # logger.info("best_index: {}, shape: {}".format(o1, o1.shape))
    # exit(0)
    providers = ["CUDAExecutionProvider"]
    ort_session = onnxruntime.InferenceSession(
    ...
    if decoder_fastertransformer:
        ...
        test(to_numpy([o0, o1]), ort_outs, rtol=1e-03, atol=1e-05)
...
if __name__ == "__main__":
    ...
    context_onnx_path = os.path.join(args.output_onnx_dir, "context.onnx")
    context_pt_path = os.path.join(args.output_onnx_dir, "context.pt")
    ...
    export_context(model, logger, context_onnx_path, context_pt_path)
