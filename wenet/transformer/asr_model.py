...
class ASRModel(torch.nn.Module):
    ...
    def forward(
        ...
        if self.ctc_weight != 1.0:
            # print("see loss before" )
            loss_att, acc_att = self._calc_att_loss(encoder_out, encoder_mask,
                                                    text, text_lengths)
            # print("see loss ", loss_att, acc_att)
        else:
            loss_att = None
    ...
    def _calc_att_loss(
        ...
        # print("see criterion_att", decoder_out, decoder_out.shape)
    ...
    def _ctc_prefix_beam_search(
        ...
        encoder_out, encoder_mask = self._forward_encoder(
            ...
            simulate_streaming)  # (B, maxlen, encoder_dim)
        if debug:
            logger.info('ori encoder_out: {} shape: {}'.format(encoder_out, encoder_out.shape))

        if context_graph is not None and context_graph.deep_biasing:
            # if debug:
            #     logger.info('see context_graph: {}'.format(context_graph))
            #     logger.info('see context_module: {}'.format(self.context_module))
            encoder_out = context_graph.forward_deep_biasing(
                encoder_out, self.context_module, self.ctc)
            if debug:
                logger.info('after deep_biasing encoder_out: {} shape: {}'.format(encoder_out, encoder_out.shape))

        maxlen = encoder_out.size(1)
        ctc_probs = self.ctc.log_softmax(
            encoder_out)  # (1, maxlen, vocab_size)
        # logger.info('ctc_probs: {} shape: {}'.format(ctc_probs, ctc_probs.shape))
        # if debug:
        #     beam_log_probs, beam_log_probs_idx = torch.topk(
        #         ctc_probs, beam_size, dim=2
        #     )
        #     print("beam_log_probs ", beam_log_probs)
        #     print("beam_log_probs_idx ", beam_log_probs_idx)

        ctc_probs = ctc_probs.squeeze(0)
        # logger.info('after squeeze ctc_probs.shape: {}'.format(ctc_probs.shape))
        ...
        for t in range(0, maxlen):
            ...
            # default value(-inf, -inf, 0, 0.0)
            next_hyps = defaultdict(lambda:
                                    (-float('inf'), -float('inf'), 0, 0.0, -float('inf'),-float('inf'),[],[],-float('inf')))
            # 2.1 First beam prune: select topk best
            top_k_logp, top_k_index = logp.topk(beam_size)  # (beam_size,)
            for s in top_k_index:
                s = s.item()
                ps = logp[s].item()
                # logger.info('s : {} ps: {}'.format(s, ps))
                ...
                # 每次更新，先更新当前序列得分，然后更新维特比得分，再更新时间戳序列，因为每个序列可能由多个子串规约得来，维特比得分记录的是概率最高的那个路径
                # 并保存该路径的时间戳，用来表示字级别时间戳
                    ...
                    if s == 0:  # blank
                        ...
                        if n_vnb < vnb + ps:
                            ...
                            if n_ctp < ps:
                                ...
                                # logger.info('case 1 abs_time_step: {}'.format(abs_time_step))
                        next_hyps[prefix] = (n_pb, n_pnb, c_state, c_score, n_vb, n_vnb, n_tb, n_tnb, n_ctp)
        ...
        #         for y in cur_hyps]
        hyps = [(y[0], log_add([y[1][0], y[1][1]]) + y[1][3], y[1][6] if y[1][4] > y[1][5] else y[1][7])
                for y in cur_hyps]
        # for i in range(len(hyps)):
        #     # logger.info('i: {} hyps[i]: {}'.format(i, hyps[i]))
        #     logger.info('i: {} hyps[i][0]: {} len(hyps[i][2]): {}'.format(i, hyps[i][0], len(hyps[i][2])))
        #     break
    ...
    def attention_rescoring(
        ...
        for i, hyp in enumerate(hyps):
            ...
            if score > best_score:
                best_score = score
                best_index = i
        # logger.info('attention rescoring hyps: {}'.format(hyps))
        # logger.info('best_index: {} best_score: {}'.format(best_index, best_score))
